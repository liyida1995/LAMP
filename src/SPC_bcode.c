...
int main(int argc, char **argv)
{
	...
	char filename[50];
	sprintf(filename,"./inputData/P1/datapoint_%d",list[0]);
 ...
 mapping_by_box(data,cnt,boxes,box_cnt);
	...
	for(int col=0;col<6;col++){
		sprintf(filename,"./inputData/P1/datapoint_%d",list[col]);
		...
		sprintf(filename,"ori_Pin_%d.bcode",list[col]);
                ...
                c_size_fpc=FPC_Compress(20,fp,fout);
                fclose(fp);
                fclose(fout);
	...
	for(i=Level-1;i>0;i--)
	{
		...
		if(fp==NULL){
	        ...
	        sprintf(filename,"LAMP_Pin_%d.bcode",list[col]);
                fout=fopen(filename,"w");
