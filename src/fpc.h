...
static const long long unsigned int mask[8] =
...
long FPC_Compress_analysic(long predsizem1, FILE *fp)
{
	long i, out, intot, hash, dhash, code, bcode, ioc;
	long long val, lastval, stride, pred1, pred2, xor1, xor2;// long: 8bytes, long long: 8bytes
	long long *fcm, *dfcm;
...
long FPC_Compress(long predsizem1, FILE *fp)
{
	long i, out, intot, hash, dhash, code, bcode, ioc;
	long long val, lastval, stride, pred1, pred2, xor1, xor2;// long: 8bytes, long long: 8bytes
	long long *fcm, *dfcm;
	...
	//ioc = fwrite(outbuf, 1, 1, stdout);
	//assert(1 == ioc);
	FILE *compress_fp=fopen("result_ori.fpc","a");
        if(compress_fp==NULL){
        printf("Cannot open file result_ori\n");
                return 0;
       }

	ioc = fwrite(outbuf, 1, 1, compress_fp);
        assert(1 == ioc);
        fclose(compress_fp);
	predsizem1 = (1L << predsizem1) - 1;
	if(predsizem1>33554431)
...
//		assert(ioc == out);
		compress_fp=fopen("result_ori.fpc","a");
                if(compress_fp==NULL){
                printf("Cannot open file result_ori\n");
                   return 0;
                }

               ioc = fwrite(outbuf, 1, out, compress_fp);
                assert(ioc == out);
                fclose(compress_fp);
		cnt+=out;
		intot = fread(inbuf, 8, SIZE, fp);
...
int main(int argc, char *argv[])
{
	...
	if (argc > 1) {
		...
		FPC_Compress(val,fp);
		gettimeofday(&end, 0);
		delta1 = (end.tv_sec - start.tv_sec) * 1000 + (end.tv_usec - start.tv_usec) / 1000.0;
		fprintf(ofp, "Compression time: %f ms\n", delta1);
		...
		Decompress();
		gettimeofday(&end, 0);
		delta2 = (end.tv_sec - start.tv_sec) * 1000 + (end.tv_usec - start.tv_usec) / 1000.0;
		fprintf(ofp, "Decompression time: %f ms\n", delta2);
	}
	fclose(ofp);
	return 0;
}*/
